...
void Database::install(object::ProcessingContext& context) const {
	if(refId.empty()) {
		...
		if(context.findObject<esl::database::ConnectionFactory>(refId) == nullptr) {
			if(id.empty()) {
				throw FilePosition::add(*this, "Could not add database reference, because referenced object with id '" + refId + "' is not a database connection factory.");
			}
			else {
				throw FilePosition::add(*this, "Could not add database reference with id '" + id + "', because referenced object with id '" + refId + "' is not a database connection factory.");
			}
		}
	}
	//context.addObject(id, create());
